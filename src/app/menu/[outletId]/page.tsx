...
import { outlets as mockOutlets, menuItems as mockMenuItems } from '@/lib/data';
...
export default function MenuPage() {
  ...
  const outlet = useMemo(() => mockOutlets.find(o => o.id === outletId), [outletId]);
  const outletImage = PlaceHolderImages.find(img => img.id === outlet?.imageId);

  useEffect(() => {
    if (outlet) {
      setMenuItems(mockMenuItems.filter(item => item.outletId === outletId));
      setOutletId(outlet.id);
    }
  }, [outletId, outlet, setOutletId]);
